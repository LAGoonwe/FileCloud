...
func GetFilesByUserName(username string, limit int) ([]BackendUserFile, error) {
	...
	// 分页限制返回数量
	stmt, err := mydb.DBConn().Prepare(
		"select user_name,file_sha1,file_name,file_size,file_abs_location,file_rel_location,upload_at,last_update,status from tbl_user_file where user_name like ? limit ?")
	if err != nil {
...
func GetFilesByFileName(filename string, limit int) ([]BackendUserFile, error) {
	...
	// 分页限制返回数量
	stmt, err := mydb.DBConn().Prepare(
		"select user_name,file_sha1,file_name,file_size,file_abs_location,file_rel_location,upload_at,last_update,status from tbl_user_file where file_rel_location like ? limit ?")
	if err != nil {
